...
function marketCardHTML(q, { interactive = true } = {}) {
  ...
  const extraStyle = instGrad ? `style="background:${instGrad}"` : '';

  let tagsHTML = `<span class="tag tag-cat" style="background:${catColor}22;border-color:${catColor}44;color:${catColor}">${esc(q.category||'כללי')}</span>`;
  ...
  if (interactive) {
    ...
    else if (dl.text) footerRight = `<span class="card-deadline">${esc(dl.text)}</span>`;
    footerRight += `<span class="card-bet-hint">הנח הימור ←</span>`;
  } else {
    const result = q.result;
    footerRight = `<span class="tag tag-resolved">נסגר · ${result === 'YES' ? yesLabel : noLabel}</span>`;
  }
  ...
  const guestVoteHTML = (!currentUser && interactive && !q.resolved) ? `
    <div class="guest-vote-row">
      <button class="guest-vote-btn yes-vote" onclick="event.stopPropagation();guestVote(${q.id},'YES')">${yesLabel}</button>
      <button class="guest-vote-btn no-vote"  onclick="event.stopPropagation();guestVote(${q.id},'NO')">${noLabel}</button>
    </div>` : '';

  const resultBanner = q.resolved ? `<div class="card-result-row ${q.result==='YES'?'yes-win':'no-win'}">תוצאה: ${q.result==='YES'?yesLabel:noLabel}</div>` : '';

  return `
<div class="market-card${q.resolved?' resolved':''}" data-id="${q.id}" data-inst="${esc(q.institution||'כללי')}" ${clickAttr} ${extraStyle}>
  <div class="card-top">
  ...
  <div class="prob-track">
    <div class="prob-yes-fill" id="pct-bar-${q.id}" style="width:${yesPct}%"></div>
  </div>
  <div class="prob-labels">
    <div class="prob-item">
      <span class="prob-pct yes" id="yes-pct-${q.id}">${yesPct}%</span>
      <span class="prob-name">${yesLabel}</span>
    </div>
    <div class="prob-divider"></div>
    <div class="prob-item">
      <span class="prob-name">${noLabel}</span>
      <span class="prob-pct no" id="no-pct-${q.id}">${noPct}%</span>
    </div>
  </div>
  ${guestVoteHTML}
  ${resultBanner}
...
function renderMarkets(questions) {
  ...
  grid.innerHTML = questions.map((q, i) => {
    const s = marketCardHTML(q);
    return s.replace('class="market-card', `class="market-card" style="animation-delay:${i * 0.05}s`).replace(`class="market-card"`, `class="market-card`);
  }).join('');

  // re-apply stagger via dataset
...
function renderLeaderboard(users) {
  ...
  el.innerHTML = users.map((u, i) => {
    ...
    const rank = i < 3 ? `<span>${medals[i]}</span>` : `<span class="lb-rank" style="color:var(--text-3)">${i+1}</span>`;
    const rankWrap = `<div class="lb-rank ${rankClass[i] || ''}">${i < 3 ? medals[i] : (i+1)}</div>`;
    const badge = i === 0 ? `<span class="lb-badge">מצטיין דיקן</span>` : '';
    const meTag = isMe ? `<span class="lb-me-tag">(אתה)</span>` : '';
    return `
<div class="lb-row${isMe ? ' me' : ''}">
  ${rankWrap}
  <span class="lb-name">${esc(u.display_name || u.username)} ${badge} ${meTag}</span>
...
function launchConfetti(x, y) {
  const colors = ['#7c6aff','#00d68f','#ff4d6d','#f5a623','#a394ff'];
  for (let i = 0; i < 22; i++) {
...
function startCountdowns() {
  ...
  window._cdTimer = setInterval(() => {
    document.querySelectorAll('[id^="dl-"]').forEach(el => {
      ...
      if (!card) return;
      // deadline stored in card? We just re-compute from global data if available
      // For simplicity: just refresh display text on next silent poll
    });
  }, 30000);
}
...
function renderComplaints(items) {
  ...
  if (!items.length) {
    el.innerHTML = `<div style="color:var(--text-3);font-size:.875rem;padding:1rem 0">אין בקשות עדיין. היה הראשון!</div>`;
    return;
  }
  ...
  <div class="admin-item-top">
    ...
    <div class="admin-item-actions">
      ${!q.resolved ? `
        ...
        <button class="btn-sm" style="background:var(--no);color:#fff" onclick="resolveQuestion(${q.id},'NO')">לא ✗</button>
      ` : ''}
...
function renderAdminActivity(items, activeType = 'all') {
  ...
  if (!items.length) { listEl.innerHTML = `<div style="color:var(--text-3);padding:1rem">אין פעילות</div>`; return; }
  const dotClass = (type) => {
